...
[CreateAssetMenu(menuName = "Create CameraState/BossRoomState")]
public class BossRoomState : CameraState
{
	[SerializeField] 
	...
	public override void Run()
	{
		...
		float dynamicSplitMagnitude = Mathf.LerpUnclamped(splitMagnitude, lateralSplitMagnitude, inv);
		...
		centroid = PlayerThis.position + centroidOffsetPosition;// Vector3.ClampMagnitude( centroidOffsetPosition, dynamicSplitMagnitude);

		// Camera zoom
		float distanceFraction = Vector3.Distance(PlayerThis.position, PlayerOther.position) * .5f / dynamicSplitMagnitude;
		topDownOffset.z = Mathf.LerpUnclamped(zoomedInDistance, zoomedOutDistance, distanceFraction);
		
		// Offset to keep both players on screen
		float zoomMultiplier = 5;
		Vector3 zoomOffset = Vector3.LerpUnclamped(Vector3.zero, new Vector3(-1, 0, -1), inv) * zoomMultiplier;
		
		CameraShake();
		
		FadeObstacles();
		
		cameraPosition = centroid + abovePlayer + zoomOffset + cameraShakeOffset + CameraTransform.rotation * topDownOffset;
	...
	public void ShakeCamera(float magnitude) => sharedTrauma += magnitude;
	...
	private void CameraShake()
	{
		...
		easedTrauma = Ease.EaseInQuad(sharedTrauma);
